...
function isLikelyOmxCliEntryPath(value: string): boolean {
...
function resolveQuestionUiProcessArgs(
  ...
  const resolvedOmxBin = resolveOmxCliEntryPath({
    argv1: process.argv[1],
    cwd: options.cwd,
    env: options.env,
  });
  const currentLauncherPath = safeString(process.argv[1]).trim();
  const omxBin = isLikelyOmxCliEntryPath(currentLauncherPath)
    ? currentLauncherPath
    : resolvedOmxBin || currentLauncherPath;
  ...
  try {
    ...
    return status
      ...
      .some((line) => {
        ...
        if (!resolvedPaneId) return paneDead === paneId;
