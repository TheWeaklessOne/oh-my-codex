...
export type MissionEvent =
  | MissionEventBase<'mission_bootstrapped', {
      mission_root: string;
      started_at: string;
      target_fingerprint: string;
    }>
  | MissionEventBase<'source_pack_prepared', {
      status: 'prepared' | 'updated';
      path: string;
      source_count: number;
      ambiguity: 'low' | 'medium' | 'high';
    }>
  | MissionEventBase<'mission_brief_prepared', {
      status: 'prepared' | 'updated';
      path: string;
      brief_id: string;
    }>
  | MissionEventBase<'acceptance_contract_prepared', {
      status: 'prepared' | 'updated';
      path: string;
      contract_id: string;
      contract_revision: number;
    }>
  | MissionEventBase<'execution_plan_prepared', {
      status: 'prepared' | 'updated';
      path: string;
      plan_id: string;
  ...
  | MissionEventBase<'workflow_stage_entered', {
      stage: string;
      detail: string;
  ...
  | MissionEventBase<'lane_summary_recorded', {
      iteration: number;
  ...
  | MissionEventBase<'iteration_committed', {
      iteration: number;
  ...
  | MissionEventBase<'mission_cancel_requested', {
      status: MissionStatus;
      reason: string | null;
    }>
  | MissionEventBase<'closeout_generated', {
      status: MissionStatus;
...
async function appendMissionEvent(missionRoot: string, event: MissionEvent): Promise<void> {
  await appendFile(missionEventsPath(missionRoot), `${JSON.stringify(event)}\n`, 'utf-8');
}

export async function loadMissionEvents(missionRoot: string): Promise<MissionEvent[]> {
...
export async function ensureMissionBootstrapEvent(mission: MissionState): Promise<void> {
  if (existsSync(missionEventsPath(mission.mission_root))) return;
...
export async function appendMissionOrchestrationEvents(
  ...
  if (changed.sourcePack) {
    await appendMissionEvent(mission.mission_root, {
      ...prefix,
      event_type: 'source_pack_prepared',
      payload: {
        status: existsSync(paths.sourcePackPath) ? 'updated' : 'prepared',
  ...
  if (changed.brief) {
    await appendMissionEvent(mission.mission_root, {
      ...
      payload: {
        status: existsSync(paths.missionBriefStatePath) ? 'updated' : 'prepared',
        path: paths.missionBriefPath,
        brief_id: artifacts.brief.brief_id,
      },
    });
  }
  if (changed.acceptanceContract) {
    await appendMissionEvent(mission.mission_root, {
      ...prefix,
      event_type: 'acceptance_contract_prepared',
      payload: {
        status: artifacts.acceptanceContract.contract_revision > 1 ? 'updated' : 'prepared',
  ...
  if (changed.executionPlan) {
    await appendMissionEvent(mission.mission_root, {
      ...prefix,
      event_type: 'execution_plan_prepared',
      payload: {
        status: artifacts.executionPlan.plan_revision > 1 ? 'updated' : 'prepared',
...
export async function appendMissionIterationCommittedEvent(
  ...
  await appendMissionEvent(mission.mission_root, {
    ...
    payload: {
      ...
      delta_path: mission.current_iteration === iteration ? join(mission.mission_root, 'iterations', String(iteration).padStart(3, '0'), 'delta.json') : null,
      final_reason: mission.final_reason,
    },
  });
}

export async function appendMissionCancelRequestedEvent(mission: MissionState, reason: string | null): Promise<void> {
  await appendMissionEvent(mission.mission_root, {
    schema_version: 1,
