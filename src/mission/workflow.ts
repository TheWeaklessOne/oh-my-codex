...
import type {
	...
	MissionPlanningApprovalMode,
	MissionPlanningMode,
	MissionPlanningTransactionStatus,
...
export interface MissionWorkflowState {
	...
	plan_run_id: string;
	plan_revision: number;
	planning_status: MissionPlanningTransactionStatus;
...
interface MissionWorkflowStageSnapshot {
	stage: MissionWorkflowStage;
	detail: string;
	iteration: number | null;
	laneType: MissionLaneType | null;
...
export async function loadMissionWorkflow(
	missionRoot: string,
	...
	switch (event.event_type) {
		...
		case "execution_plan_prepared": {
			return {
				...
				handoff_surface: event.payload.handoff_surface as
					| "plan"
					| "ralplan"
					| "deep-interview",
				strategy_key: event.payload.strategy_key,
				blocked_reason:
					event.payload.plan_status === "blocked"
						? event.payload.blocking_reason
						: null,
				artifact_refs: {
					...state.artifact_refs,
		...
		case "planning_transaction_recorded":
			if (
				state.strategy_history.at(-1)?.plan_run_id === event.payload.plan_run_id
			) {
				return {
					...state,
					updated_at: event.recorded_at,
					plan_run_id: event.payload.plan_run_id,
					plan_id: event.payload.plan_id,
					plan_revision: event.payload.plan_revision,
					planning_status:
						event.payload.status as MissionPlanningTransactionStatus,
					approval_mode:
						event.payload.approval_mode as MissionPlanningApprovalMode,
					approved_at: event.payload.approved_at,
					approved_by: event.payload.approved_by,
					replan_reason: event.payload.replan_reason,
					strategy_key: event.payload.strategy_key,
				};
			}
			return {
				...
				strategy_history: [
					...state.strategy_history,
					{
						...
						reason: event.payload.replan_reason
							?? (event.payload.previous_plan_run_id
...
function workflowStageSnapshotFromState(
	state: MissionWorkflowState,
): MissionWorkflowStageSnapshot {
	const last = state.stage_history.at(-1);
	return {
		stage: state.current_stage,
		detail: last?.detail ?? "",
		iteration: state.current_iteration,
		laneType: state.current_lane,
		blockedReason: state.blocked_reason,
	};
}

function workflowStageSnapshotFromOptions(
	options: SyncMissionWorkflowOptions,
): MissionWorkflowStageSnapshot {
	return {
		stage: options.stage,
		detail: options.detail,
		iteration: options.iteration ?? null,
		laneType: options.laneType ?? null,
		blockedReason: options.blockedReason ?? null,
	};
}

function sameWorkflowStageSnapshot(
	left: MissionWorkflowStageSnapshot,
	right: MissionWorkflowStageSnapshot,
): boolean {
	return (
...
export async function reconcileMissionWorkflow(mission: MissionState): Promise<{
	...
	const driftDetected =
		JSON.stringify(current ?? null) !== JSON.stringify(rebuilt);
	if (driftDetected) {
...
export async function syncMissionWorkflow(
	options: SyncMissionWorkflowOptions,
): Promise<MissionWorkflowState> {
	const current = await rebuildMissionWorkflowFromEvents(options.mission);
	if (
		!sameWorkflowStageSnapshot(
			workflowStageSnapshotFromState(current),
			workflowStageSnapshotFromOptions(options),
		)
	) {
