...
export function buildTeamInstruction(
  ...
  return `${launchCommand} # task=${JSON.stringify(descriptor.task)} # policy=${descriptor.staffingPlan.constraints.policy} # role_contract=${descriptor.roleContract ?? 'shared-agent-type'} # hardening=${descriptor.staffingPlan.constraints.hardening} # staffing=${descriptor.staffingPlan.staffingSummary} # verify=${descriptor.staffingPlan.verificationPlan.summary}`;
}
