...
import {
	type PreparedMissionRuntime,
	prepareMissionRuntime,
} from "../mission/runtime.js";
import { loadMissionV3ArtifactRoles } from "../mission/v3.js";
...
Behavior:
  ...
  - bootstraps Mission V2-compatible source grounding artifacts plus Mission V3 assurance/control artifacts before launch
...
function inferSourceKind(ref: string): MissionRequirementSourceInput["kind"] {
	if (/\.md$/i.test(ref))
		return /spec|prd|requirement/i.test(ref) ? "spec" : "doc";
	if (/issue|ticket|bug|incident|github\.com\/.+\/issues\/\d+/i.test(ref))
		return "issue";
...
async function collectMissionRequirementSources(
	...
	const repoRootRealPath = await realpath(repoRoot).catch(() => repoRoot);
	...
	for (const file of bootstrap.sourceFiles) {
		...
		if (!isPathInsideRoot(repoRootRealPath, filePath)) {
			...
			sources.push({
				...
				trustLevel: "low",
				partialFailureReason: `Source file escapes repo root and cannot be ingested: ${file}`,
			});
			continue;
		}
		...
		if (!isPathInsideRoot(repoRootRealPath, realFilePath)) {
			const content = `Mission source file unavailable: ${file}`;
			sources.push({
				kind: inferSourceKind(file),
				title: `File source: ${file}`,
				...
				origin: "internal",
				adapter: "cli-file",
				sourceUri: `file://${realFilePath}`,
				fetchedAt: collectedAt,
				contentHash: `content:${hashValue(content)}`,
				retrievalStatus: "partial_failure",
				freshnessTtlSeconds: 300,
				trustLevel: "low",
				partialFailureReason: `Source file resolves outside repo root and cannot be ingested: ${file}`,
			});
			continue;
		}
...
function buildMissionTargetFingerprint(
	parsed: MissionCliParseResult,
	requirementSources: MissionRequirementSourceInput[],
): string {
	return `mission-cli:${hashValue(
		JSON.stringify({
			task: parsed.task,
			desiredOutcome: parsed.bootstrap.desiredOutcome ?? "",
			constraints: parsed.bootstrap.constraints,
			unknowns: parsed.bootstrap.unknowns,
			touchpoints: parsed.bootstrap.touchpoints,
			highRisk: parsed.bootstrap.highRisk,
			sources: requirementSources.map((source) => ({
				kind: source.kind,
				refs: source.refs,
				sourceUri: source.sourceUri ?? null,
				contentHash: source.contentHash ?? null,
				retrievalStatus: source.retrievalStatus ?? null,
				trustLevel: source.trustLevel ?? null,
				partialFailureReason: source.partialFailureReason ?? null,
			})),
		}),
	)}`;
}

export function parseMissionCliArgs(
	args: readonly string[],
): MissionCliParseResult {
	const words: string[] = [];
...
function buildMissionAppendInstructions(
	task: string,
	runtime: PreparedMissionRuntime | null,
): string {
	return [
		"<mission_cli_mode>",
		...
		runtime
			? `Acceptance contract: ${runtime.artifactPaths.acceptanceContractPath}`
			: null,
		runtime
			? `Execution plan: ${runtime.artifactPaths.executionPlanPath}`
			: null,
		runtime
			? `Assurance contract: ${runtime.v3Paths.assuranceContractPath}`
			: null,
		runtime ? `Proof program: ${runtime.v3Paths.proofProgramPath}` : null,
		runtime
			? `Environment contract: ${runtime.v3Paths.environmentContractPath}`
			: null,
		runtime
			? `Planning status: ${runtime.planning.status} via ${runtime.planning.handoffSurface}`
			: null,
		runtime ? `Mission V3 lifecycle: ${runtime.mission.lifecycle_state}` : null,
		runtime?.planning.blockingReason
			? `Blocking reason: ${runtime.planning.blockingReason}`
			: null,
...
async function bootstrapMissionCliLaunch(
	cwd: string,
	parsed: MissionCliParseResult,
): Promise<PreparedMissionRuntime> {
	const repoRoot = tryResolveRepoRoot(cwd);
	const requirementSources = await collectMissionRequirementSources(
		cwd,
		parsed.bootstrap,
	);
	const slug = slugifyMissionTask(parsed.task);
	const targetFingerprint = buildMissionTargetFingerprint(
		parsed,
		requirementSources,
	);
...
async function writeMissionAppendixFile(
	cwd: string,
	task: string,
	runtime: PreparedMissionRuntime | null,
): Promise<string> {
	...
	await writeFile(
		appendixPath,
		`${buildMissionAppendInstructions(task, runtime)}\n`,
		"utf-8",
	);
	return appendixPath;
}
...
function formatMissionInspectView(view: MissionInspectView): string {
	const lines = [
		...
		"",
		"Artifacts:",
		...view.artifactRoles.map(
			(artifact) => `- [${artifact.role}] ${artifact.path}`,
		),
	];
	return lines.join("\n");
}

async function buildMissionInspectView(
	cwd: string,
	slug: string,
): Promise<MissionInspectView> {
	const repoRoot = tryResolveRepoRoot(cwd);
	...
	const v3Artifacts = await loadMissionV3ArtifactRoles(
		mission.mission_root,
		mission.active_candidate_id ?? mission.selected_candidate_id ?? undefined,
	);
	return {
		slug: mission.slug,
		...
		artifactRoles: [
			{
				path: join(mission.mission_root, "mission.json"),
				role: "authoritative",
			},
			{ path: join(mission.mission_root, "latest.json"), role: "derived" },
			{
				path: join(mission.mission_root, "events.ndjson"),
				role: "append_only",
			},
...
export async function missionCommand(
	...
	const runtime = await (
		dependencies.bootstrapMission ?? bootstrapMissionCliLaunch
	)(cwd, parsed);
	...
	try {
		...
		const missionPrompt =
			task === "mission-cli-launch" ? "$mission" : `$mission ${task}`;
